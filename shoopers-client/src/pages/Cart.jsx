  ...
  return (
    <div className="h-full py-4">
      <div className="flex flex-col">
        {cartItems.length > 0 && (
          <div className="flex flex-col pb-8 md:flex-row justify-between px-6 items-center">
            <p className="hidden md:block relative cursor-pointer group">
              ...
              <Link to="/">
                <span className=" opacity-0 group-hover:opacity-100 absolute top-0 left-0 bg-green-400 text-black capitalize py-1 px-4 rounded-md text-xs shadow-[5px_5px_0px_0px_rgba(109,40,217)]">
                  Back to shopping
                </span>
              </Link>
            </p>
        ...
        {/* Move the empty cart message and image outside the grid */}
        {cartItems.length === 0 && (
        ...
        {/* Render the grid only if there are items in the cart */}
        {cartItems.length > 0 && (
            <div className="md:py-4 grid md:grid-cols-3 md:px-6  gap-6">
              {cartItems.map((item) => (
                <div key={item._id}>
                  <div
